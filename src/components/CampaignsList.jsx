...
import { getAllCampaignsSync, getAllCampaigns } from "@/utils/campaigns";
...
export default function CampaignsList() {
  ...
  useEffect(() => {
    // Charger les campagnes enrichies
    getAllCampaigns()
      .then((enrichedCampaigns) => {
        setCampaigns(enrichedCampaigns);
        setLoading(false);
      })
      .catch((error) => {
        console.error("Erreur lors du chargement des campagnes:", error);
        // Fallback sur les campagnes brutes en cas d'erreur
        setCampaigns(getAllCampaignsSync());
        setLoading(false);
      });
  }, []);

  const filteredCampaigns =
    activeFilter === "all" ? campaigns : campaigns.filter((c) => c.slug === activeFilter);
    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={navStyle} className="sticky-nav">
        <button
          ...
          onMouseEnter={(e) => {
            if (activeFilter !== "all") e.currentTarget.style.background = "rgba(255,255,255,0.1)";
          }}
          onMouseLeave={(e) => {
            if (activeFilter !== "all") e.currentTarget.style.background = "rgba(255,255,255,0.05)";
          }}
        >
          🔥 Dernières Campagnes
        </button>
        {campaigns.map((c) => (
          <button
            ...
            onMouseEnter={(e) => {
              if (activeFilter !== c.slug) e.currentTarget.style.background = "rgba(255,255,255,0.1)";
            }}
            onMouseLeave={(e) => {
              if (activeFilter !== c.slug) e.currentTarget.style.background = "rgba(255,255,255,0.05)";
            }}
          >
            {c.name}
          </button>
